...
class DoctorWang(BaseDoctor):
    ...
    def diagnose(self, frame, index, name):
        ...
        scale_factor = {'body':[1.0,1.1],'disc':[0.9,1.4]}
    ...
    def locate_keypoint_from_mask(self, pred_mask, point_num, threshold,name):
        """
        ...
        if Flag is not True or name == 'study252':
            if name =='study252':
                distance_list = compute_distance_list(keypoint_result)
                if max(distance_list) > 1.5 * min(distance_list):
                    index_max = distance_list.index(max(distance_list))
                    x = (keypoint_result[index_max][0] - keypoint_result[index_max+1][0])//2
                    y = (keypoint_result[index_max][1] - keypoint_result[index_max+1][1])//2
                    new_point = [keypoint_result[index_max][0] - x, keypoint_result[index_max][1] - y]
                    keypoint_result.insert((index_max+1),new_point)
                    # print('Lost at inner.')
        ...
        test_mask = torch.zeros_like(pred_mask)
        for point in keypoint_result:
            y,x = point
            test_mask[x][y] = torch.tensor(1)
        kernel = np.ones((5,5))
        score_mask = torch.tensor(signal.convolve2d(test_mask.numpy(), kernel, boundary='symm', mode='same'))

        return keypoint_result, int(np.mean(compute_distance_list(keypoint_result))),score_mask
    ...
    def get_body_disc_keypoint_dict(self,image,name):
        ...
        body_keypoint_list,body_average,result_mask_body = self.locate_keypoint_from_mask(body_pred, self.params.body_keypoint,
                                                              self.params.body_threshold,name='None')
        disc_keypoint_list,disc_average,result_mask_disc = self.locate_keypoint_from_mask(disc_pred, self.params.disc_keypoint,
                                                              self.params.disc_threshold,name)
        # self.visdom.register(torch.tensor(image[0])+result_mask_body, 'heatmap', 2, 'body_pred_heatmap_process')
        # self.visdom.register(torch.tensor(image[0])+result_mask_disc, 'heatmap', 2, 'disc_pred_heatmap_process')
        # for visualize
        # plt.figure(1)
        # plt.subplot(1, 1, 1)
        # plt.imshow(result_mask_body.numpy()+image[0])
        # plt.savefig('/home/adminer/SPARK/PyDoctor/pydoctor/result_plots/body_loc_B/' + name + '.png')
        # plt.show()
        # #
        # plt.figure(1)
        # plt.subplot(1, 1, 1)
        # plt.imshow(result_mask_disc.numpy()+image[0])
        # plt.savefig('/home/adminer/SPARK/PyDoctor/pydoctor/result_plots/disc_loc_B/' + name + '.png')
        # plt.show()
    ...
    def generate_bbox_label(self, location_list, scale_factor,size,argument_number,frame,resize_sz=64):
        ...
        patches_return = bbox_list.view(-1,3,64,64)
