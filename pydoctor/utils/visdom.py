...
class VisImages(VisBase):
    ...
    def draw_data(self):
        self.visdom.images(self.raw_data.clone(), opts={'title': self.title}, win=self.title)
...
class VisContour(VisBase):
    ...
    def draw_data(self):
        self.visdom.contour(self.raw_data.clone(),  opts={'title': self.title}, win=self.title)

class VisSurf(VisBase):
    def __init__(self, visdom, show_data, title):
        super().__init__(visdom, show_data, title)
    ...
    def draw_data(self):
        self.visdom.surf(self.raw_data.clone(),  opts={'title': self.title}, win=self.title)
    ...
    def register(self, data, mode, debug_level=0, title='Data', **kwargs):
        if title not in self.registered_blocks.keys():
            ...
            if mode == 'image':
                ...
                self.registered_blocks[title] = VisImages(self.visdom,show_data,title)
            elif mode == 'heatmap':
                ...
                self.registered_blocks[title] = VisSurf(self.visdom,show_data,title)
            elif mode == 'contour':
                self.registered_blocks[title] = VisContour(self.visdom,show_data,title)
            else:
